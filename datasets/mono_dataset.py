...
class MonoDataset(data.Dataset):
    def __init__(self,
        ...
        self.num_scales = num_scales  # 多尺度处理的尺度数
        self.interp = Image.ANTIALIAS  # 图像缩放时使用的插值方式
        self.frame_idxs = frame_idxs  # 帧索引列表，用于选择图像帧
        self.is_train = is_train  # 是否为训练模式
        self.img_ext = img_ext  # 图像文件扩展名
        # 图像加载器和转换器
        self.loader = pil_loader  # PIL 图像加载器
        self.to_tensor = transforms.ToTensor()  # 图像转换为张量的转换器
        # 尝试使用新版本的 torchvision 来指定图像增强参数
        try:
            # 如果 torchvision 支持新的元组版本，设置颜色增强的范围
